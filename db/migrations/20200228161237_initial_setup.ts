...
export async function up(knex: Knex): Promise<any> {
  ...
  await knex.schema.createTable('user_tags', t => {
    t.uuid('user_id').references('users.id').onDelete('cascade')
    t.specificType('tag_id', 'serial').references('tags.id').onDelete('cascade')
  ...
  await knex.schema.createTable('mentors', t => {
    t.uuid('id').primary().references('users.id').onDelete('cascade')
  })

  await knex.schema.createTable('time_slots', t => {
    t.uuid('id').primary()
    t.uuid('mentor_id').references('mentors.id').onDelete('cascade')
    t.dateTime('start').notNullable()
    t.dateTime('end')
  })

  await knex.schema.createTable('meetups', t => {
    t.uuid('slot_id').primary().references('time_slots.id').onDelete('cascade')
    t.enum('status', ['pending', 'confirmed', 'cancelled'], {
      useNative: true,
      enumName: 'meetup_status',
    }).defaultTo('pending')
    t.uuid('mentee_id').references('users.id').onDelete('cascade')
    t.string('message')
    t.string('location')
  })
}
