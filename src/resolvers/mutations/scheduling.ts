...
export const updateSlots = async (
  ...
  const addList = added.map(({ start, end }, i) => {
    ...
    Object.entries({ start, end }).map(([k, v]) => {
      if (isNaN(v.getTime()))
        throw new UserInputError(`invalid ${k} time "${start}" in added[${i}]`)
    })
...
export const requestSlot = async (_, { input }, { id }) => {
...
export const acceptMeetup = async (_, { meetupId }, { id }) => {
  if (!id) throw new AuthenticationError('Not logged in.')
...
export const cancelMeetup = async (_, { meetupId }, { id }) => {
